 ...
 *========================================================================*/
static const char mapx_c_rcsid[] = "$Header: /tmp_mnt/FILES/mapx/mapx.c,v 1.25 1998-03-05 00:00:46 knowles Exp $";
...
/* iteration control */
#define IH_MAXITS 30
#define IH_EPSILON  1.0e-10

/* region boundaries */
#ifdef IT_WORKED
#define IH_phi1  0.710987989993 
#define IH_phi2  0.0            
#define IH_phi3 -0.710987989993 
#define IH_lam1 -1.74532925199  
#define IH_lam2 -0.349065850399 
#define IH_lam3  1.3962634016   
#define IH_lam4 -0.698131700798 
#endif
...
/* mystery constants */
#define IH_mc1 0.900316316158
#define IH_mc2 1.4142135623731
...
static int interupted_homolosine_equal_area(float lat, float lon, float *u, float *v)
{
  int it, region;
  double x, y, phi, lam, cos_phi, sin_phi, delta_lam;
  double feast, theta, delta_theta, constant;
  ...
  if (phi >= IH_phi1)	             /* if on or above 40 44' 11.8" */
  {
    if (lam <= IH_lam4) region = 0;   /* If to the left of -40 */
    else region = 2;
  }
  else if (phi >= IH_phi2)			     /* Between 0.0 and 40 44' 11.8" */
  {
    if (lam <= IH_lam4) region = 1;   /* If to the left of -40 */
    else region = 3;
  }
  else if (phi >= IH_phi3)   	     /* Between 0.0 & -40 44' 11.8" */
  {
    if (lam <= IH_lam1) region = 4;  	/* If between -180 and -100 */
    else if (lam <= IH_lam2) region = 5;/* If between -100 and -20 */
    else if (lam <= IH_lam3) region = 8;	/* If between -20 and 80 */
    else region = 9;				/* If between 80 and 180 */
  }
  else						/* Below -40 44' */
  {
    if (lam <= IH_lam1) region = 6;       /* If between -180 and -100 */
    else if (lam <= IH_lam2) region = 7; /* If between -100 and -20 */
    else if (lam <= IH_lam3) region = 10;   /* If between -20 and 80 */
    else region = 11;                            /* If between 80 and 180 */
  }

  delta_lam = lam - IH_lam0[region];
  RNORMALIZE(delta_lam);
  feast = current->Rg * IH_lam0[region];
 
  if (region==1||region==3||region==4||region==5||region==8||region==9)
  {
    x = feast + current->Rg * delta_lam * cos_phi;
    y = current->Rg * phi;
  }
  else
  {
 ...
 */
    for (it=0; it < IH_MAXITS; it++)
    {
      delta_theta = -(theta + sin(theta) - constant) / (1.0 + cos(theta));
      theta += delta_theta;
      if (fabs(delta_theta) < IH_EPSILON) break;
    }
    ...
    theta /= 2.0;
    x = feast + IH_mc1 * current->Rg * delta_lam * cos(theta);
    y = current->Rg * (IH_mc2*sin(theta) - IH_mc3*sign(phi));
  }
...
static int inverse_interupted_homolosine_equal_area(float u, float v, float *lat, float *lon)
{
  int region;
  double x, y, xor, yor, lam, phi, theta, arg, feast; 
 ...
 *	get region
 */
  if (yor >= IH_phi1)  /* if on or above 40 44' 11.8" */
  {
    if (xor <= IH_lam4) region = 0; /* If to the left of -40 */
    else region = 2;
  }
  else if (yor >= IH_phi2)                      /* Between 0.0 and 40 44' 11.8" */
  {
    if (xor <= IH_lam4) region = 1; /* If to the left of -40 */
    else region = 3;
  }
  else if (yor >= IH_phi3)           /* Between 0.0 & -40 44' 11.8" */
  {
    if (xor <= IH_lam1) region = 4;     /* If between -180 and -100 */
    else if (xor <= IH_lam2) region = 5; /* If between -100 and -20 */
    else if (xor <= IH_lam3) region = 8;  /* If between -20 and 80 */
    else region = 9;                             /* If between 80 and 180 */
  }
  else                                            /* Below -40 44' 11.8" */
  {
    if (xor <= IH_lam1) region = 6;     /* If between -180 and -100 */
    else if (xor <= IH_lam2) region = 7; /* If between -100 and -20 */
    else if (xor <= IH_lam3) region = 10; /* If between -20 and 80 */
    else region = 11;                            /* If between 80 and 180 */
  }
  ...
  if (region==1||region==3||region==4||region==5||region==8||region==9)
  {
    ...
    if (fabs(fabs(phi) - PI/2) > IH_EPSILON)
    {
  ...
  {
    arg = (y + IH_mc3*current->Rg * sign(y)) / (IH_mc2*current->Rg);
    if (fabs(arg) > 1.0) return -1;
    theta = asin(arg);
    lam = IH_lam0[region] + x/(IH_mc1*current->Rg*cos(theta));
    if (lam < -PI) return -1;
 ...
 */
  switch (region)
  { case 0: case 1: if (lam < -PI || lam > IH_lam4) return -1; break;
    case 2: case 3: if (lam < IH_lam4 || lam > PI) return -1; break;
    case 4: case 6: if (lam < -PI || lam > IH_lam1) return -1; break;
    case 5: case 7: if (lam < IH_lam1 || lam > IH_lam2) return -1; break;
    case 8: case 10: if (lam < IH_lam2 || lam > IH_lam3) return -1; break;
    case 9: case 11: if (lam < IH_lam3 || lam > PI) return -1; break;
  }
