...
*/
{
  ...
  if (lon_cen_mer < -TWOPI  ||  lon_cen_mer > TWOPI) { 
    error("isinusforinit", "bad parameter; longitude of central meridian invalid");
    return ISIN_ERROR;
  }
  ...
  if (fabs(djustify - ijustify) > EPS_CNVT) {
    error("isinusforinit", "bad parameter; ijustify not near an integer value");
    return ISIN_ERROR;
  }
...
*/
{
  ...
  if (sphere < EPS_SPHERE) 
    {istat = Isin_error(&ISIN_BADPARAM, "Isin_for_init"); return NULL;}
  ...
  if (this == NULL) 
    {istat = Isin_error(&ISIN_BADALLOC, "Isin_for_init"); return NULL;}
  ...
  if (this->ref_lon < -PI) this->ref_lon += TWOPI;
  ...
  if (this->row == NULL) {
    free(this);
    istat = Isin_error(&ISIN_BADALLOC, "Isin_for_init");
    return NULL;
  }
  ...
  for (irow = 0; irow < this->nrow_half; irow++, row++) {
    ...
    clat = HALFPI * (1.0 - ((double) irow + 0.5) / this->nrow_half);
  ...
  this->col_dist = (TWOPI * sphere) / ncol_cen;
  this->col_dist_inv = ncol_cen / (TWOPI * sphere);
...
!Team Unique Header:
 ...
 ! Usage Notes:
   ...
   2. The longitude must be in the range [-'TWOPI' to 'TWOPI'].
   3. The latitude must be in the range [-'HALFPI' to 'HALFPI'].
...
!Output Parameters:
 ...
 (returns)      status:
		  ISIN_SUCCESS - normal return
                  ISIN_ERANGE - longitude or latitude not in range
...
*/
{
  ...
  *y = 0.0;
  ...
  if (this == NULL) return Isin_error(&ISIN_BADHANDLE, "Isin_fwd");
  if (this->key != ISIN_KEY) return Isin_error(&ISIN_BADKEY, "Isin_fwd");
  if (lon < -TWOPI  ||  lon > TWOPI) return ISIN_ERANGE;
  if (lat < -HALFPI  ||  lat > HALFPI) return ISIN_ERANGE;
  ...
  *y = this->false_north + (lat * this->sphere);
  ...
  row = (HALFPI - lat) * this->ang_size_inv;
  irow = row;
  ...
  if (flon < 0.0) flon += (1 - (long) flon);
  if (flon > 1.0) flon -= (long) flon;
...
*/
{
  if (this == NULL) return Isin_error(&ISIN_BADHANDLE, "Isin_for_free");
  if (this->key != ISIN_KEY) return Isin_error(&ISIN_BADKEY, "Isin_for_free");
