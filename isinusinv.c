...
*/
{
  ...
  if (sphere < EPS_SPHERE) 
    {istat = Isin_error(&ISIN_BADPARAM, "Isin_inv_init"); return NULL;}
  ...
  if (this == NULL) 
    {istat = Isin_error(&ISIN_BADALLOC, "Isin_inv_init"); return NULL;}
  ...
  if (this->row == NULL) {
    free(this);
    istat = Isin_error(&ISIN_BADALLOC, "Isin_inv_init");
    return NULL;
  }
...
int Isin_inv(const Isin_t *this, 
	     double x, double y,
...
*/
{
  ...
  *lat = 0.0;
  ...
  if (this == NULL) return Isin_error(&ISIN_BADHANDLE, "Isin_inv");
  if (this->key != ISIN_KEY) return Isin_error(&ISIN_BADKEY, "Isin_inv");
...
*/
{
  if (this == NULL) return Isin_error(&ISIN_BADHANDLE, "Isin_inv_free");
  if (this->key != ISIN_KEY) return Isin_error(&ISIN_BADKEY, "Isin_inv_free");
